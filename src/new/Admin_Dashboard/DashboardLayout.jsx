...
export default function DashboardLayout() {
    ...
    switch (accountType) {
      case "Driver":
        specificButtons = [
          ...
          {
            screen: "DriverCompletedLoads",
            label: "My Completed Loads",
            icon: <FaDatabase />,
          },
        ];
        break;
  ...
  return (
    <div className={styles.container}>
        ...
        {buttons.map((button, index) => (
          <button
            ...
            className={`${styles.sidebarButton} ${
              activeScreen === button.screen ? styles.active : ""
            }`}
            onClick={button.action || (() => setActiveScreen(button.screen))}
            aria-label={button.label}
          >
      ...
      <main className={styles.mainContent}>{renderScreen()}</main>
    </div>
  );
}
